...
export const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    <motion.div 
      ...
      className={`glass-card p-8 flex flex-col h-full relative overflow-hidden group border hover:border-[var(--color-muted-text)] transition-colors ${isFeatured ? 'border-white/10 shadow-2xl' : 'border-[var(--color-border)]'}`}
    >
      {isFeatured && (
      ...
      {subtitle && <p className="text-sm font-semibold mb-4 relative z-10 opacity-90">{subtitle}</p>}
      <div className="text-[var(--color-muted-dark-text)] mb-6 flex-grow relative z-10 leading-relaxed">
        {description}
      </div>
      ...
      {to && (
        <div className="relative z-10 mt-auto pt-4 border-t border-white/5">
          <Link to={to} className={`font-medium flex items-center transition-all ${accentColorClass} hover:brightness-125 group-hover:gap-2`}>
