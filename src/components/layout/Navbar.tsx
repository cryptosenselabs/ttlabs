...
export const Navbar = () => {
  ...
  const navLinkClass = (path: string) => {
    const base = "transition-all duration-200 text-[15px] font-medium px-4 py-2 rounded-full";
    return isActive(path) 
      ? `${base} bg-slate-100 text-[#0F172A]`
      : `${base} text-[#475569] hover:text-[#0F172A] hover:bg-slate-50`;
  };

  return (
    <nav className="fixed w-full z-50 bg-white/95 backdrop-blur-md border-b border-[#E2E8F0] transition-all duration-300">
      <div className="max-w-[1120px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-[72px]">
          ...
          <div className="md:hidden flex items-center">
            <button onClick={toggleMenu} className="text-[#475569] hover:text-[#0F172A] focus:outline-none transition-colors p-2 bg-slate-50 rounded-lg">
